...
class TrasladosController extends base_controller_1.BaseController {
    ...
    initRoutes() {
        ...
        this.router.get(`${this.basePath}/traslados/getListaUnidadesConfiguradasPost/:idCotizacion`, this.getListaUnidadesConfiguradasPost.bind(this));
        ...
        // TrasladosPosteriores
        this.router.get(`${this.basePath}/traslados/getTrasladosPosteriores/:idCotizacion`, this.getTrasladosPosteriores.bind(this));
        this.router.post(`${this.basePath}/traslados/guardaTrasladoPorVinPosteriores`, this.guardaTrasladoPorVinPosteriores.bind(this));
    }
    ...
    getListaUnidadesConfiguradasPost(request, response) {
        const trasladoBusiness = new catalogo_1.TrasladoBusiness();
        const idCotizacion = request.params.idCotizacion;
        trasladoBusiness.getListaUnidadesConfiguradasPost(idCotizacion).then((res) => {
            response.status(200).send(res);
        }, (err) => {
            response.status(500).send(err);
        });
    ...
    // Traslados Posteriores Consulta
    getTrasladosPosteriores(request, response) {
        const trasladoBusiness = new catalogo_1.TrasladoBusiness();
        const idCotizacion = request.params.idCotizacion;
        trasladoBusiness.getTrasladosPosteriores(idCotizacion).then((Traslados) => {
            response.status(200).send(Traslados);
        }, (err) => {
            response.status(500).send(err);
        });
    }
    guardaTrasladoPorVinPosteriores(request, response) {
        // const idUsuario = Number((request.headers.idusuario) ? request.headers.idusuario : 0);
        const traslado = request.body;
        const idCotizacion = traslado.idCotizacionSelect;
        const idTraslado = traslado.idTrasladosSelect;
        const vin = traslado.vinSelect;
        const cotizacionBusiness = new catalogo_1.TrasladoBusiness();
        cotizacionBusiness.guardaTrasladoPorVinPosteriores(idCotizacion, idTraslado, vin).then((res) => {
