...
class TrasladoBusiness {
    ...
    // OCT 99 20201214 lista unidades configuradas para armar traslados posteriores
    getListaUnidadesConfiguradasPost(idCotizacion) {
        console.log(idCotizacion);
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            const connectionVRC = typeorm_1.getConnection();
            const queryRunnerVRC = connectionVRC.createQueryRunner();
            yield queryRunnerVRC.connect();
            try {
                const respuesta = yield queryRunnerVRC.query(`exec sp_Flotillas_ListadoUnidadesConfiguradasPost @idCotizacion = '${idCotizacion}'`);
                resolve(respuesta);
            }
            catch (error) {
                reject(error);
    ...
    // Traslados Posteriores Guarda
    guardaTrasladoPorVinPosteriores(idCotizacion, idTraslado, vin) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            const connectionVRC = typeorm_1.getConnection();
            const queryRunnerVRC = connectionVRC.createQueryRunner();
            yield queryRunnerVRC.connect();
            try {
                const respuesta = yield queryRunnerVRC.query(`exec sp_flotillas_guarda_trasladoPorVin_posteriores @idCotizacion = '${idCotizacion}' , @idTraslado = '${idTraslado}'  , @vin = '${vin}'`);
                resolve(respuesta);
            }
            catch (error) {
                reject(error);
    ...
    getTrasladosPosteriores(idCotizacion) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            const connection = typeorm_1.getConnection();
            yield connection.query(`exec sp_flotillas_trasladosPost @idCotizacion='${idCotizacion}'`)
                .then((resultadosTrasladosPosteriores) => {
                const trasladosPosterior = [];
                resultadosTrasladosPosteriores.map((resultado) => {
                    trasladosPosterior.push({ idTraslado: resultado.idTraslado, nombre: resultado.nombre });
                });
                resolve(trasladosPosterior);
            })
                .catch(reject);
        }));
    }
