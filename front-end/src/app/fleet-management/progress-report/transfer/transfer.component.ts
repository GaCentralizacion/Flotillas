import { Component, OnInit, Input,ViewEncapsulation, ViewChild, QueryList, ViewChildren, ContentChildren, ANALYZE_FOR_ENTRY_COMPONENTS } from '@angular/core';
...
export class TransferComponent implements OnInit {
  ...
  ngOnInit() {    
    ...
    //this.pricingService.getPricingById(this.idCotizacion).subscribe((cotizacion: Cotizacion) => {
    this.pricingService.getGrupoUnidadByIdCotizacion(this.idCotizacion).subscribe((cotizacion: Cotizacion) => {
      // lleno el arreglo de unidades vin
      const unidadesVin = cotizacion.gruposUnidades[0].detalleUnidades;
      let Data = [];
      for (let i = 0; i < unidadesVin.length; i++) {
        Data.push(unidadesVin[i].vin);
      }
      this.unidadesVin = Data;
      //OCT99
      this.pricingService.getUnidadesInteresByIdCotizacion(this.idCotizacion).subscribe((cotizacion: Cotizacion) => {
  ...
  agregarTraslado(opcion) {
    ...
    //console.log('>>>> Edita ', traslado.idCfdi);
    ...
    this.trasladoService.saveTraslado(this.idPuntoDePartida, this.idPuntoDeLLegada, traslado).subscribe(item => {
      ...
      //this.ActualizaTrasladoFree();
    }
      , err => this.toasterService.error('Error al actualizar: ' + err.error.message + ' query:  ' + err.error.query)
    );    
  }

  // ActualizaTrasladoFree(){
  //   this.pricingService.isFreeTransfer(this.cotizacion.idMarca, this.cotizacion.idCotizacion).subscribe((res: any) => {
