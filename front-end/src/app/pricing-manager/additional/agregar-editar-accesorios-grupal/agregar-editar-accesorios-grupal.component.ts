...
@Component({
  ...
  templateUrl: './agregar-editar-accesorios-grupal.component.html',
  styleUrls: ['./agregar-editar-accesorios-grupal.component.scss']
})

export class AgregarEditarAccesoriosGrupalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.idGrupoUnidad !== undefined) {
      ...
      this.pricingService.getAdicionalesGestionbyGrupal(this.cotizacion.idCotizacion, this.idGrupoUnidad).subscribe((adicionales: any) => {
        ...
        this.grupoSelected.accesoriosSinPaquete = adicionales[0];
    ...
    this.buscarPartidasSisco(noPlantaSisco)
      .then((res: any[]) => {
        ...
        if (nuevosSisco.length > 0) {
          ...
          AgregaNuevoSisco.agregarExternoPostAd(agregarSisco);
        }
        if (agregarCatalogoSisco.length > 0) {
          CatalogoSisco.cotizacion = this.cotizacion;
          CatalogoSisco.idGrupoUnidad = this.idGrupoUnidad;

          CatalogoSisco.agregaCatalogoSiscoDesdeFueraPostAd(agregarCatalogoSisco);
        }
      })
      .then(() => {
        if (planta.length > 0) {
          ...
          if (this.idGrupoUnidad) {
            ...
            this.pricingService.saveGestionUnidadAccesorioGrupal(this.cotizacion.idCotizacion,
              this.idGrupoUnidad, this.idDetalleUnidad, agregadoNormal)
              .subscribe((res) => {
          ...
          if (agregar.length > 0) {
            ...
            this.pricingService.saveGestionUnidadAccesorioGrupal(this.cotizacion.idCotizacion,
              this.idGrupoUnidad, this.idDetalleUnidad, agregar)
              .subscribe((res) => {
                this.activeModal.close(true);
                this.toastrService.success(`Accesorios agregados al grupo ${this.idGrupoUnidad} de forma sactisfactoria.`, `OPERACION EXITOSA`);
              }, error => {
  ...
  abrirCatalogoSisco() {
    ...
    modalCatalogoSisco.componentInstance.modalGestoria = true;
  ...
  abrirAgregarSisco() {
    ...
    modalAgregarAccesorioSisco.componentInstance.modalGestoria = true;
  ...
  */
  ...
  precioBloqueado(row) {
    //console.log('validaCosto: ',row);
    let validacion = true; //desactivado    
  //  if (this.cotizacion.tipoOrden == 'CU') {
      if (row.Origen === 'FLO') {
        validacion = false;
      }
      if (row.Origen === 'BPRO') {
    ...
    // }
    // if (this.cotizacion.tipoOrden == 'FI') {
    //   if (row.Origen === 'FLO') {
    //     validacion = false;
    //   }
    //   if (row.Origen === 'BPRO') {
